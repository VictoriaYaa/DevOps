...
mode_pods() {
    ...
    if [ "$NAME" ]; then
        namespaces=($(echo "$data" | \
                      ...
                      sort -u))
#        namespaces=($(echo "$data" | \
#                      jq -r ".items[] | select(.metadata.labels.agni_job_type==\"$NAME\") | \
#                             .metadata.namespace" | \
#                      sort -u))
...
#            pods=($(echo "$nsdata" | \
#                    jq -r "select(.status.reason!=\"Evicted\" \
#                                   and .metadata.labels.agni_job_type===\"$NAME\") | \
#                           .metadata.name"))
    else
            pods=($(echo "$nsdata" | \
                    jq -r "select(.status.reason!=\"Evicted\") | \
    ...
    if [ -z "$ns" ]; then
        ...
        OUTPUT="$OUTPUT$count_ready pods ready, $count_succeeded pods succeeded, $count_failed pods not ready|ready=$count_ready;;;; succeeded=$count_succeeded;;;; failed=$count_failed"
    fi
}
...
mode_deployments() {
    ...
    # deflate the data
    data="$(echo "$rawdata" | jq -r '.items[] | {name: .metadata.name, namespace: .metadata.namespace, status: .status.conditions, replicas: .spec.replicas, availableReplicas: .status.availableReplicas}')"
    ...
    # put the name of the application to variable and check if it's exist in the json nsdata with filter available True\False, also checks if it has replicas more than availableReplicas
    # If Available True => availdeps, If False => progressdeps
    for ns in "${namespaces[@]}"; do
        nsdata="$(echo "$data" | jq -c -r "select(.namespace==\"$ns\")")"
        availdeps=($(echo "$nsdata" | jq -r "{name: .name , replicas: .replicas , availableReplicas: .availableReplicas , status: .status[]|select(.type==\"Available\")} | \
                                             select(.status.status == \"True\") | .name"))
        progressdeps=($(echo "$nsdata" | jq -r "{name: .name , replicas: .replicas , availableReplicas: .availableReplicas , status: .status[]|select(.type==\"Available\")} | select(.status.status == \"False\") | select(.replicas> .availableReplicas) | .name"))
        #Collecting the information of the Available=False
        progressdep_replica="$(echo "$nsdata" | jq -r "{name: .name , replicas: .replicas , availableReplicas: .availableReplicas , status: .status[]|select(.type==\"Available\")} | select(.status.status == \"False\") | select(.replicas> .availableReplicas)")"
        ...
        for dep in "${deps[@]}"; do
            availreplica=$(echo "$progressdep_replica" | jq ".  | select(.name == \"$dep\") | select(.replicas > .availableReplicas ) | .availableReplicas")
            replicas=$(echo "$progressdep_replica" | jq ".  | select(.name == \"$dep\") | select(.replicas > .availableReplicas) | .replicas")
            if [[ " ${availdeps[@]} " =~ " $dep " ]]; then
                ((count_avail++))
                availdeps_replica=($(echo "$nsdata" | jq -r "{name: .name , replicas: .replicas , availableReplicas: .availableReplicas , status: .status[]|select(.type==\"Available\")} | select(.status.status == \"True\") | select(.name == \"$dep\") | .availableReplicas"))
                performance_data="$performance_data available_replicas=$availdeps_replica;"
            elif [[ " ${progressdeps[@]} " =~ " $dep " && $availreplica != null ]]; then
                ((count_failed++))
                EXITCODE=1
                OUTPUT="Deployment $ns/$dep has $availreplica available replicas out of $replicas"
                performance_data="$performance_data available_replicas=$availreplica;"
            else
                ((count_failed++))
                EXITCODE=2
                OUTPUT="Deployment $ns/$dep"
                performance_data="$performance_data available_replicas=0;"
            fi
        done
    done
    ...
    if [ $EXITCODE = 0 ]; then
        if [ -z "$ns" ]; then
            ...
            if [ $count_avail -gt 1 ]; then
                OUTPUT="OK. $count_avail deployments are available$performance_data"
		PERF="$PERF available=$count_avail;;;;"
            else
                OUTPUT="OK. Deployment available$performance_data"
            fi
        fi
    elif [ $EXITCODE = 1 ]; then
        if [ $count_failed = 1 ]; then
            OUTPUT="Status is Progressing: $OUTPUT$performance_data"
        else
            OUTPUT="$OUTPUT and $((--count_failed)) more are not available$performance_data"
	    PERF="$PERF failed=$((--count_failed));;;;"
        fi
    else
        if [ $count_failed = 1 ]; then
            OUTPUT="$OUTPUT not available$performance_data"
        else
            OUTPUT="$OUTPUT and $((--count_failed)) more are not available$performance_data"
	    PERF="$PERF failed=$((--count_failed));;;;"
        fi
...
mode_daemonsets() {
    ...
    OUTPUT="$OUTPUT|available=${statusArr[numberReady]};;;; desired=${statusArr[desiredNumberScheduled]};;;;"
}
...
mode_jobs() {
    ...
    OUTPUT="$OUTPUT|total=$total_jobs;;;; failed=${total_failed_count};;;;"
}
